...
deliver(Job) ->
    ...
    MaxAttempts = maps:get(<<"max_attempts">>, Job, hl_core_retry:max_attempts()),
        ...
        {ok, Event, Endpoint} ->
            ...
            case Enabled of
                false ->
                    ...
                    hl_store_client:nack_job(JobId, 300);
                _ ->
            ...
            case hl_delivery_rate:check(EndpointId, RateRPS) of
                {error, rate_limited} ->
                    logger:warning(#{event => delivery_rate_limited,
                                     job_id => JobId, endpoint_id => EndpointId}),
                    hl_delivery_metrics:inc_rate_limited(EndpointId),
                    hl_store_client:nack_job(JobId, 5);

                ok ->
                    %% Acquire semaphore (max_in_flight)
                    case hl_delivery_sem:acquire(EndpointId, MaxInFlight) of
                        {error, at_capacity} ->
                            logger:info(#{event => delivery_at_capacity,
                                          job_id => JobId, endpoint_id => EndpointId}),
                            hl_store_client:nack_job(JobId, 1);
                        ...
                        ok ->
                            ...
                            hl_delivery_sem:release(EndpointId),
...
%% ── Actor delivery path ──────────────────────────────────────────────────────
%% Called from a spawned process in hl_endpoint_actor:dispatch_next/1.
...
deliver_actor(Job, Endpoint) ->
    ...
    TenantId   = maps:get(<<"tenant_id">>, Job),
    Transform  = maps:get(<<"transform">>, Job, null),
    Transformed = hl_core_transform:apply(Transform, Event),
    AttemptId  = hl_core_uuid:generate_str(),
...
actor_result(Job, _Other) ->
    ...
    case AttemptN >= MaxAtt of
        ...
        false -> {retry, hl_core_retry:backoff_secs(AttemptN, #{}),
...
record_attempt(AttemptId, JobId, EventId, EndpointId, TenantId, N,
    ...
    hl_delivery_metrics:observe_latency(EndpointId, Duration),
    hl_store_client:append_attempt(#{
        <<"attempt_id">>   => AttemptId,
        <<"job_id">>       => JobId,
        <<"event_id">>     => EventId,
...
publish_delivery_status(TenantId, AttemptId, JobId, EventId, EndpointId,
    ...
    hl_stream_pubsub:publish(TenantId, #{
...
handle_result(JobId, EventId, EndpointId, TenantId, AttemptN, MaxAttempts, Result) ->
    case Result of
        {ok, Status, _} when Status >= 200, Status < 300 ->
            ...
            hl_delivery_metrics:inc_delivered(TenantId, EndpointId);
...
retry_job(JobId, EventId, EndpointId, TenantId, AttemptN, MaxAttempts) ->
    case AttemptN >= MaxAttempts of
        ...
        false ->
            Delay = hl_core_retry:backoff_secs(AttemptN, #{}),
            logger:info(#{event => delivery_retry_scheduled,
                          job_id => JobId, attempt_n => AttemptN, delay_secs => Delay}),
            hl_store_client:nack_job(JobId, Delay),
            hl_delivery_metrics:inc_retried(TenantId, EndpointId)
    end.

dlq_job(JobId, EventId, EndpointId, TenantId, AttemptN, Reason) ->
    hl_store_client:put_dlq(#{
        <<"job_id">>        => JobId,
        <<"event_id">>      => EventId,
        <<"endpoint_id">>   => EndpointId,
        <<"tenant_id">>     => TenantId,
    ...
    hl_store_client:ack_job(JobId),
    hl_delivery_metrics:inc_dlq(TenantId, EndpointId).
