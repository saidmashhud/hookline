-module(gp_api_h_dev_inbox).
-export([init/2]).

init(Req0, #{action := ui} = Opts) ->
    PrivDir = code:priv_dir(gp_dev_inbox),
        ...
        MsgId ->
            ...
            case lists:filter(fun(M) -> maps:get(<<"id">>, M, undefined) =:= MsgId end, All) of
                [Msg | _] ->
                    ...
                    {ok, Req, Opts};
                [] ->
                    gp_api_error:reply(Req0, 404, not_found, <<"Message not found">>),
                    {ok, Req0, Opts}
            end
    end;
...
inline_ui_html() ->
    <<"<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\">",
      "<title>GatePulse Dev Inbox</title>",
      ...
      "<script>",
      "let token = localStorage.getItem('gp_inbox_token') || '';",
      "document.getElementById('tokenInput').value = token;",
      "if(token) { showUrl(); loadMessages(); }",
      "async function createInbox() {",
      "  const r = await fetch('/v1/dev/inbox', {method:'POST',headers:{'Authorization':'Bearer dev-secret','Content-Type':'application/json'},body:'{}'});",
      "  const d = await r.json();",
      "  token = d.token; localStorage.setItem('gp_inbox_token', token);",
      "  document.getElementById('tokenInput').value = token;",
      ...
      "  localStorage.setItem('gp_inbox_token', token);",
      "  if(!token) return;",
      "  showUrl();",
      "  const r = await fetch('/v1/dev/inbox/messages?token='+token, {headers:{'Authorization':'Bearer dev-secret'}});",
      "  const d = await r.json();",
      "  const msgs = d.items || [];",
