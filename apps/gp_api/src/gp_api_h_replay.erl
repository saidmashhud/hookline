-module(gp_api_h_replay).
-export([init/2, init_table/0]).

-define(REPLAY_TABLE, gp_replay_results).
    ...
    case cowboy_req:binding(id, Req0) of
        ...
        ReplayId ->
            case ets:lookup(?REPLAY_TABLE, ReplayId) of
                ...
                [] ->
                    gp_api_error:reply(Req0, 404, not_found, <<"Replay not found">>),
                    {ok, Req0, Opts}
            end
    end;
...
replay_single(TId, EventId, Req, Opts) ->
    case gp_store_client:get_event(TId, EventId) of
        ...
        {error, _} ->
            gp_api_error:reply(Req, 404, not_found, <<"Event not found">>),
            {ok, Req, Opts}
    end.
...
replay_batch(TId, Params, Req, Opts) ->
    ...
    case gp_store_client:list_events(TId, StoreOpts) of
        {ok, #{<<"items">> := AllEvents}} ->
            Events1 = case Topic of
                undefined -> AllEvents;
                P -> [E || E <- AllEvents,
                           gp_core_topic:matches(P, maps:get(<<"topic">>, E, <<>>))]
            end,
        ...
        {error, R} ->
            gp_api_error:reply(Req, 500, store_error, iobin(R)),
            {ok, Req, Opts}
    end.
    ...
    Results = lists:map(fun(Event) ->
        ...
        case gp_core:route_event(TId, Event) of
            {ok, Subs} ->
                ...
                {ok, JobIds} = gp_core:create_delivery_jobs(Event, FilteredSubs, TId),
