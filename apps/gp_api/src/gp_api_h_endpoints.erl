...
handle(<<"POST">>, Req0, Opts) ->
    ...
    case jsx:decode(Body, [return_maps]) of
        Map when is_map(Map) ->
            case gp_core_schema:validate_endpoint(Map#{<<"tenant_id">> => TId}) of
                ok ->
                    EndpointId = gp_core_uuid:generate_str(),
                ...
                {error, {invalid_field, F, Reason}} ->
                    ...
                    gp_api_error:reply(Req1, 400, validation_error, Msg),
                    {ok, Req1, Opts};
                {error, _} ->
                    gp_api_error:reply(Req1, 400, validation_error, <<"Invalid endpoint data">>),
                    {ok, Req1, Opts}
            end;
        _ ->
...
handle(<<"GET">>, Req0, Opts) ->
    ...
    case cowboy_req:binding(id, Req0) of
        undefined ->
            case gp_store_client:list_endpoints(TId) of
        ...
        EndpointId ->
            case gp_store_client:get_endpoint(EndpointId) of
                ...
                {error, <<"not found">>} ->
                    gp_api_error:reply(Req0, 404, not_found, <<"Endpoint not found">>),
                    {ok, Req0, Opts};
                {error, R} ->
                    gp_api_error:reply(Req0, 500, store_error,
                        list_to_binary(io_lib:format("~p", [R]))),
...
handle(<<"PATCH">>, Req0, Opts) ->
    ...
    if EndpointId =:= undefined ->
        gp_api_error:reply(Req0, 400, validation_error, <<"endpoint id required">>),
        {ok, Req0, Opts};
    true ->
        {ok, Body, Req1} = cowboy_req:read_body(Req0),
        TId = get_tenant(Req1),
        case jsx:decode(Body, [return_maps]) of
            Delta when is_map(Delta) ->
                case gp_store_client:get_endpoint(EndpointId) of
                    ...
                    {error, <<"not found">>} ->
                        gp_api_error:reply(Req1, 404, not_found, <<"Endpoint not found">>),
                        {ok, Req1, Opts};
                    {error, R} ->
                        gp_api_error:reply(Req1, 500, store_error,
                            list_to_binary(io_lib:format("~p", [R]))),
