...
handle(<<"GET">>, <<"/v1/admin/stats">>, Req0, Opts) ->
    ...
    QueueDepth = case hl_store_client:queue_stats(TenantId) of
        {ok, #{<<"pending">> := P, <<"inflight">> := I}} ->
            #{<<"pending">> => P, <<"inflight">> => I};
        _ ->
            #{<<"pending">> => 0, <<"inflight">> => 0}
    end,
    CompactionStats = case catch hl_compaction:stats() of
...
handle(<<"GET">>, <<"/v1/admin/audit">>, Req0, Opts) ->
    ...
    case hl_store_client:list_audit(TenantId, #{<<"limit">> => Limit}) of
        {ok, #{<<"items">> := Items}} ->
