#!/usr/bin/env bash
# GatePulse — complex end-to-end test scenarios
# Usage: GP_URL=http://localhost:8080 GP_API_KEY=change-me-in-production bash test/e2e.sh
set -euo pipefail

GP_URL="${GP_URL:-http://localhost:8080}"
GP_API_KEY="${GP_API_KEY:-change-me-in-production}"
...
gp_patch() { curl -sf -X PATCH  -H "Authorization: Bearer $GP_API_KEY" \
               -H "Content-Type: application/json" -d "$2" "${GP_URL}$1"; }
...
wait_for_delivery() {
  ...
  for i in $(seq 1 "$timeout"); do
    sleep 1
    local cnt
    cnt=$(gp_get "/v1/dev/inbox/messages?token=$token" 2>/dev/null \
          | jq_len ".get('items',[])")
...
sleep 3
CNT2=$(gp_get "/v1/deliveries?event_id=$ID_A" | jq_len ".get('items',[])")
if [[ "$CNT2" -le 2 ]]; then
...
gp_post "/v1/subscriptions" \
  "{\"endpoint_id\":\"$EP2_ID\",\"topic_pattern\":\"payments.#\"}" >/dev/null

gp_post "/v1/events" '{"topic":"payments.processed","payload":{"ref":"p1"}}' >/dev/null
gp_post "/v1/events" '{"topic":"payments.failed","payload":{"ref":"p2"}}' >/dev/null
# This should NOT be delivered (different top-level topic)
gp_post "/v1/events" '{"topic":"orders.paid","payload":{"ref":"p3"}}' >/dev/null

sleep 5
CNT3=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN2" | jq_len ".get('items',[])")
if [[ "$CNT3" -eq 2 ]]; then
...
INBOX3=$(gp_post "/v1/dev/inbox" '{}')
TOKEN3=$(echo "$INBOX3" | jq_str "['token']")
RECV3=$(echo "$INBOX3" | jq_str "['receive_url']")
EP3=$(gp_post "/v1/endpoints" \
  "{\"url\":\"$RECV3\",\"enabled\":true,\"timeout_ms\":3000}")
EP3_ID=$(echo "$EP3" | jq_str "['endpoint_id']")
gp_post "/v1/subscriptions" \
  "{\"endpoint_id\":\"$EP3_ID\",\"topic_pattern\":\"ship.*\"}" >/dev/null

gp_post "/v1/events" '{"topic":"ship.created","payload":{}}' >/dev/null  # ✓
gp_post "/v1/events" '{"topic":"ship.updated","payload":{}}' >/dev/null  # ✓
gp_post "/v1/events" '{"topic":"ship.label.printed","payload":{}}' >/dev/null  # ✗ (two segments)

sleep 5
CNT_STAR=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN3" | jq_len ".get('items',[])")
...
gp_post "/v1/subscriptions" "$(cat <<JSON
{
  "endpoint_id": "$EP4_ID",
  "topic_pattern": "sales.*",
  "filter": {
...
gp_post "/v1/events" '{"topic":"sales.completed","payload":{"amount":200,"currency":"USD"}}' >/dev/null
# Should NOT deliver (amount=50 < 100)
gp_post "/v1/events" '{"topic":"sales.completed","payload":{"amount":50,"currency":"USD"}}' >/dev/null
# Should NOT deliver (currency=EUR)
gp_post "/v1/events" '{"topic":"sales.completed","payload":{"amount":500,"currency":"EUR"}}' >/dev/null

sleep 5
CNT4=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN4" | jq_len ".get('items',[])")
if [[ "$CNT4" -eq 1 ]]; then
...
EP5=$(gp_post "/v1/endpoints" \
  "{\"url\":\"$RECV_URL\",\"enabled\":true,\"timeout_ms\":3000,\"rate_limit_rps\":10}")
EP5_ID=$(echo "$EP5" | jq_str "['endpoint_id']")

# Disable endpoint
gp_patch "/v1/endpoints/$EP5_ID" '{"enabled":false}' >/dev/null

EP5_UPDATED=$(gp_get "/v1/endpoints/$EP5_ID")
...
# Re-enable
gp_patch "/v1/endpoints/$EP5_ID" \
  '{"enabled":true,"rate_limit_rps":100,"max_in_flight":20}' >/dev/null
EP5_FINAL=$(gp_get "/v1/endpoints/$EP5_ID")
...
EV6=$(gp_post "/v1/events" '{"topic":"replay.test","payload":{"v":1}}')
EV6_ID=$(echo "$EV6" | jq_str "['event_id']")
sleep 4

# Replay it — should trigger a second delivery
gp_post "/v1/replay" "{\"event_id\":\"$EV6_ID\"}" >/dev/null
sleep 4

CNT6=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN5" | jq_len ".get('items',[])")
if [[ "$CNT6" -ge 2 ]]; then
...
gp_post "/v1/events" '{"topic":"admin.paused","payload":{"test":true}}' >/dev/null
sleep 4
CNT_PAUSED=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN6" | jq_len ".get('items',[])")
...
CNT_RESUMED=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN6" | jq_len ".get('items',[])")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  ...
  -d "$BIG_PAYLOAD" "${GP_URL}/v1/events")
if [[ "$STATUS" == "413" ]]; then
...
STATUS_401=$(curl -s -o /dev/null -w "%{http_code}" "${GP_URL}/v1/events?limit=1")
...
STATUS_401B=$(curl -s -o /dev/null -w "%{http_code}" \
  -H "Authorization: Bearer wrong-key" "${GP_URL}/v1/events?limit=1")
...
if [[ -n "$NEW_TOKEN" ]]; then
  STATUS_NEW=$(curl -s -o /dev/null -w "%{http_code}" \
    -H "Authorization: Bearer $NEW_TOKEN" "${GP_URL}/v1/events?limit=1")
...
if [[ -n "$NEW_KEY_ID" ]]; then
  gp_del "/v1/apikeys/$NEW_KEY_ID" >/dev/null 2>&1 \
...
sleep 10
CNT_BULK=$(gp_get "/v1/dev/inbox/messages?token=$TOKEN_BULK" | jq_len ".get('items',[])")
...
curl -s -N \
  ...
  "${GP_URL}/v1/stream?topic=sse.test" > "$SSE_OUT" &
SSE_PID=$!

sleep 1  # wait for connection to establish

gp_post "/v1/events" '{"topic":"sse.test","payload":{"sse":true}}' >/dev/null
...
PAGE1=$(gp_get "/v1/events?limit=3&topic=page.test")
...
if [[ -n "$CURSOR" && "$CURSOR" != "None" && "$CURSOR" != "" ]]; then
  PAGE2=$(gp_get "/v1/events?limit=3&topic=page.test&cursor=$CURSOR")
...
section "Cleanup"
gp_del "/v1/subscriptions/$SUB_ID" >/dev/null 2>&1 && pass "Cleanup: subscription deleted" || true
gp_del "/v1/endpoints/$EP_ID"      >/dev/null 2>&1 && pass "Cleanup: endpoint deleted"    || true
